...
export default async function BookUniversePage({ params }: PageProps) {
  ...
  return (
    <div>
      <Breadcrumb rootHref="/library/books" rootLabel="Books" crumbs={[{ label: universe.name }]} />
      ...
      <div className="flex items-start justify-between mb-8 gap-4">
        ...
        <div className="flex items-start gap-2 flex-shrink-0">
          ...
          <DeleteEntityButton
            apiPath={`/api/books/universes/${universe.id}`}
      ...
      {universe.series.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">Add a series to start grouping books in {universe.name}.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {universe.series.map((s) => (
            <EntityCard key={s.id} href={`/library/books/s/${s.id}`} name={s.name} icon={Layers} meta={count(s._count.books, "book")} />
          ))}
        </div>
      )}
    </div>
