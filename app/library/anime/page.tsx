    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{count(total, "season")} in your library</p>
        </div>
        <div className="flex items-center gap-2">
          <MirrorCoversButton apiPath="/api/anime/mirror-covers" />
          <Link href="/library/anime/u/new" className="flex items-center gap-2 border border-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors">
            <Plus className="w-4 h-4" /> Universe
          </Link>
          <Link href="/library/anime/s/new" className="flex items-center gap-2 border border-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors">
...
async function AnimeContent({ status, language, q }: { status?: string; language?: string; q?: string }) {
  const statusFilter = asEnum(status, ANIME_STATUS_VALUES);
  const languageFilter = asEnum(language, LANGUAGE_VALUES);
  // A filter searches the whole library, hierarchy included — otherwise a title inside a
  ...
  const statsRows = await db.anime.findMany({
    select: { status: true, episodes: true, episodesWatched: true, episodeDuration: true, timesRewatched: true },
  });
...
async function FilteredGrid({
  ...
  statusFilter: (typeof ANIME_STATUS_VALUES)[number] | undefined;
  languageFilter: (typeof LANGUAGE_VALUES)[number] | undefined;
  q?: string;
}) {
  const filtered = await db.anime.findMany({
    where: {
...
async function Buckets() {
  ...
  const [universes, looseSeries, looseAnime] = await Promise.all([
    db.animeUniverse.findMany({
      orderBy: { name: "asc" },
      include: { series: { select: { id: true, _count: { select: { anime: true } } } } },
    }),
    ...
    db.anime.findMany({ where: { seriesId: null }, select: CARD_FIELDS, orderBy: { createdAt: "desc" } }),
  ]);

  if (universes.length === 0 && looseSeries.length === 0 && looseAnime.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <p className="text-gray-400 text-lg font-medium">Nothing here yet</p>
        <p className="text-gray-400 text-sm mt-1">Add an anime, or group things with a series or universe.</p>
      </div>
    );
  }
    ...
    <div className="space-y-10">
        ...
        <section>
          ...
          <div className={GRID}>
            {universes.map((u) => (
              <EntityCard
                key={u.id}
                href={`/library/anime/u/${u.id}`}
                name={u.name}
                icon={Globe}
                meta={`${count(u.series.length, "series", "series")} · ${count(u.series.reduce((s, x) => s + x._count.anime, 0), "season")}`}
              />
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div className={GRID}>
            {looseSeries.map((s) => (
              <EntityCard
                key={s.id}
                href={`/library/anime/s/${s.id}`}
                name={s.name}
                icon={Layers}
                meta={count(s._count.anime, "season")}
              />
            ))}
          </div>
        </section>
      ...
      {looseAnime.length > 0 && (
        <section>
          ...
          <div className={GRID}>
            {looseAnime.map((anime) => <AnimeCard key={anime.id} anime={anime} />)}
          </div>
        </section>
      )}
    </div>
