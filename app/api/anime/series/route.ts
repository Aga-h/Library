...
const createSchema = z.object({
  ...
  // Optional: a series with no universe lives on the anime main page.
...
async function POSTHandler(request: NextRequest) {
  ...
  if (universeId && !(await db.animeUniverse.findUnique({ where: { id: universeId } }))) {
    return NextResponse.json({ error: "Universe not found" }, { status: 404 });
  }

  try {
    const series = await db.animeSeries.create({
