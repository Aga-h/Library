...
async function GETHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  const universe = await db.animeUniverse.findUnique({ where: { id } });
...
async function PATCHHandler(request: NextRequest, { params }: RouteContext) {
  const { id } = await params;
  if (!(await db.animeUniverse.findUnique({ where: { id } }))) {
  ...
  const d = result.data;
  try {
    const updated = await db.animeUniverse.update({
      where: { id },
      data: {
...
async function DELETEHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  // SetNull, not cascade — the series inside survive and become standalone.
  await db.animeUniverse.delete({ where: { id } });
  revalidateTag("library-stats", "max");
