...
async function GETHandler() {
  return NextResponse.json(await db.animeUniverse.findMany({ orderBy: { name: "asc" } }));
}
...
async function POSTHandler(request: NextRequest) {
  ...
  try {
    const universe = await db.animeUniverse.create({ data: { name, notes: result.data.notes || null } });
