...
async function GETHandler() {
  return NextResponse.json(await db.bookUniverse.findMany({ orderBy: { name: "asc" } }));
}
...
async function POSTHandler(request: NextRequest) {
  ...
  try {
    const universe = await db.bookUniverse.create({ data: { name, notes: result.data.notes || null } });
