...
async function GETHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  const universe = await db.bookUniverse.findUnique({ where: { id } });
...
async function PATCHHandler(request: NextRequest, { params }: RouteContext) {
  const { id } = await params;
  if (!(await db.bookUniverse.findUnique({ where: { id } }))) {
  ...
  const d = result.data;
  try {
    const updated = await db.bookUniverse.update({
      where: { id },
      data: {
...
async function DELETEHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  await db.bookUniverse.delete({ where: { id } });
  revalidateTag("library-stats", "max");
