...
export async function POST() {
  ...
  const all = await db.movie.findMany({ select: { id: true, coverImage: true } });
  ...
  await Promise.allSettled(
    batch.map(async (item) => {
      try {
        const path = `movies/${item.id}.jpg`;
        const newUrl = await mirrorCover(item.coverImage!, path);
        await db.movie.update({ where: { id: item.id }, data: { coverImage: newUrl } });
        mirrored++;
      } catch (e) {
