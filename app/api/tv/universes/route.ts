...
async function GETHandler() {
  return NextResponse.json(await db.tvUniverse.findMany({ orderBy: { name: "asc" } }));
}
...
async function POSTHandler(request: NextRequest) {
  ...
  try {
    const universe = await db.tvUniverse.create({ data: { name, notes: result.data.notes || null } });
