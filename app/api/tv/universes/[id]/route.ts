...
async function GETHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  const universe = await db.tvUniverse.findUnique({ where: { id } });
...
async function PATCHHandler(request: NextRequest, { params }: RouteContext) {
  const { id } = await params;
  if (!(await db.tvUniverse.findUnique({ where: { id } }))) {
  ...
  const d = result.data;
  try {
    const updated = await db.tvUniverse.update({
      where: { id },
      data: {
...
async function DELETEHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  await db.tvUniverse.delete({ where: { id } });
  revalidateTag("library-stats", "max");
