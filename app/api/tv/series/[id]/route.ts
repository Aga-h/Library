...
async function GETHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  const series = await db.tvSeries.findUnique({ where: { id } });
...
async function PATCHHandler(request: NextRequest, { params }: RouteContext) {
  ...
  if (d.universeId && !(await db.tvUniverse.findUnique({ where: { id: d.universeId } }))) {
    return NextResponse.json({ error: "Universe not found" }, { status: 404 });
  }

  try {
    const updated = await db.tvSeries.update({
      where: { id },
      data: {
...
async function DELETEHandler(_r: NextRequest, { params }: RouteContext) {
  ...
  await db.tvSeries.delete({ where: { id } });
  revalidateTag("library-stats", "max");
