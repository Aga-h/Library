...
export async function POST() {
  ...
  const all = await db.tvShow.findMany({ select: { id: true, coverImage: true } });
  const pending = all.filter(t => !!t.coverImage && !isSupabaseCover(t.coverImage));
  ...
  await Promise.allSettled(
    batch.map(async (item) => {
      try {
        const path = `tv/${item.id}.jpg`;
        const newUrl = await mirrorCover(item.coverImage!, path);
        await db.tvShow.update({ where: { id: item.id }, data: { coverImage: newUrl } });
        mirrored++;
      } catch (e) {
